...
namespace ariziolouzada.espacodebeleza.pages.servico
{
    public partial class newedit : Page
    {
        ...
        private void CarregaDados(int id)
        {
            try
            {
                var srv = ServicoEspBel.Pesquisar(id);
                if (srv != null)
                {
        ...
        private bool VerificaCampos()
        {
            try
            {
                ...
                if (id.Equals("0"))
                {
                    ...
                    var srv = ServicoEspBel.Pesquisar(txtDescricao.Value, int.Parse(idEmpresaContratante));
                    if (srv != null)
                    {
                        txtDescricao.Focus();
                        ltlMsn.Text = "<div class=\"alert alert-block alert-danger fade in\"><button data-dismiss=\"alert\" class=\"close\" type=\"button\">×" +
        ...
        private bool SalvarDados()
        {
            try
            {
                ...
                var srv = new ServicoEspBel(txtDescricao.Value, decimal.Parse(txtValor.Value.Replace("R$", "")));
                ...
                if (id.Equals("0"))
                {
                    ...
                    idEmpresaContratante = Criptografia.Decrypt(idEmpresaContratante);
                    srv.IdEmpresaContratante = int.Parse(idEmpresaContratante);               

                    if (!ServicoEspBel.Inserir(srv))
                    {
                        ltlMsn.Text = "<div class=\"alert alert-block alert-danger fade in\"><button data-dismiss=\"alert\" class=\"close\" type=\"button\">X" +
